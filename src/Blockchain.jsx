...
const connectWallet =async() =>{
    try{
        ...
        reportError(error)
        // costom callbacks , rethrow them to event handler 
    }
}
...
const isWalletConnected= async() =>{
    try{
        ...
        window.ethereum.on('accountsChanged',async()=>{ // accnt[0] is bydefault selected one
            setGlobalState('connectedAccount',accounts[0].toLowerCase())
...
const getEthereumContract= async () =>{
    ...
    if(connectedAccount){
        ...
        if(networkData){
            // similar to get contract factory 
...
const structuredProposals=(proposals)=>{
    return proposals.map((proposal)=>({
        ...
        downvotes:  Number(proposal.downvotes), // must be in number format
